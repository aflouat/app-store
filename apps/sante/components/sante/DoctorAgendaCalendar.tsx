...
function addDays(dateStr: string, n: number): string {
  ...
  return toLocalISO(new Date(y, mo - 1, da + n))
}
...
export default function DoctorAgendaCalendar({ doctorId }: Props) {
  ...
  const fetchWeek = useCallback(async (monday: string) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/sante/doctor/slots?week=${monday}`)
  ...
  async function toggleCell(date: string, time: string) {
    ...
    if (existing) {
      const res = await fetch(`/api/sante/doctor/slots/${existing.id}`, { method: 'DELETE' })
      if (res.ok) {
      ...
      const res = await fetch('/api/sante/doctor/slots', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ slot_date: date, slot_time: time, duration_min: 30 }),
      })
      if (res.ok) {
  ...
  async function duplicateWeek() {
    ...
    const res = await fetch('/api/sante/doctor/slots/bulk', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // doctorId used to avoid unused-var warning — component is scoped to the authenticated doctor
  void doctorId
    ...
    <div className="cal-wrap">
      ...
      <div className="cal-grid-wrap">
        <div className="cal-grid">
          ...
          {weekDays.map((d, i) => (
            <div key={d} className={`cal-cell cal-day-header${d === today ? ' cal-day-today' : ''}`}>
              ...
              {weekDays.map(date => {
                ...
                return (
                  <div
                    ...
                    title={
                      isBooked ? 'Réservé par un patient — impossible de modifier'
      ...
      <div className="cal-legend">
        <span className="cal-legend-item">
          <span style={{ background:'#16a34a', color:'#fff', padding:'1px 6px', borderRadius:3, fontSize:'.72rem', fontWeight:700 }}>✓</span>
          Disponible (cliquer pour supprimer)
        </span>
        <span className="cal-legend-item">
          <span style={{ background:'#e07b54', color:'#fff', padding:'1px 5px', borderRadius:3, fontSize:'.65rem', fontWeight:700, letterSpacing:'.03em' }}>Pris</span>
          Réservé par un patient
        </span>
        <span className="cal-legend-item">
          <span style={{ width:8, height:8, borderRadius:'50%', background:'var(--border)', display:'inline-block', border:'1px solid var(--border)' }} />
      ...
      <style>{`
        ...
        .cal-dup-btn {
          ...
          border: 1.5px solid var(--c1); border-radius: var(--radius-sm);
        ...
        .cal-error {
          ...
          background: #fdf0ef; padding: .5rem .75rem; border-radius: 6px;
        }
        ...
        .cal-day-header {
          background: var(--bg); padding: .5rem .3rem;
        ...
        .cal-time-label {
          background: var(--bg); font-size: .73rem; color: var(--text-light);
          display: flex; align-items: center; justify-content: center;
          padding: 0 .3rem; height: 38px; font-weight: 500;
        }
        ...
        .cal-spinner {
          ...
          border: 2px solid var(--border); border-top-color: var(--c1);
          border-radius: 50%; animation: cal-spin .6s linear infinite;
        }
