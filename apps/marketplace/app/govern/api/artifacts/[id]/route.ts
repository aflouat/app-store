...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (status) {
    await updateArtifactStatus(id, status, actor_id, note)
  }
  ...
  if (toUpdate.length) {
    ...
    await query(`UPDATE governance.artifacts SET ${sets}, updated_at = NOW() WHERE id = $1`, [id, ...vals])
  }
